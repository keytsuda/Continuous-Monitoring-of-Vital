...
from mimic_gruode_KLanneal import train_gruode_KLanneal_mimic
import argparse
import os
...
#hyper_dict = {"dropout_rate": [0.2], "weight_decay":[0.0001], "lambda":[0,0.1,1], "annealer":["Constant","Linear","Warmup","Cyclical"]}
hyper_dict = {"dropout_rate": [0.1], "weight_decay":[0.0001], "lambda":[1], "annealer":["Constant","Linear","Warmup","Cyclical"]}
...
val_idx = np.load(dir_path+f"fold_idx_{fold}/val_idx.npy")
test_idx = np.load(dir_path+f"fold_idx_{fold}/test_idx.npy")
...
def main():
    ...
    for c in combinations:
        ...
        simulation_name = f"Xval_{type}_GRU_ODE_VAE_MIMIC_dropout{dropout_rate}_weightdecay{weight_decay}_lambda{lambda_factor}_anneal{annealer}_fold{fold}"
        ...
        params_dict["annealer"] = annealer
        params_dict["impute"] = False

        last_val_results, max_val_metric, test_loglik, test_auc, test_mse =train_gruode_KLanneal_mimic(simulation_name = simulation_name,
