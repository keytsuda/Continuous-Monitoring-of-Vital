...
class GRUObservationCellLogvar(torch.nn.Module):
    ...
    def forward(self, h, p, X_obs, M_obs, i_obs):
        ...
        i_obs=i_obs.long() # fujimori_追加
        p_obs        = p[i_obs] #i_obsはbatch内の患者ID

        # 各サンプルの変数について平均、log対数を算出するようにしている？
        ...
        # error=(X_obs-mean) : (観測値-予測値)として
        # lossesはモデルにmean,sigmaを予測させたうえで、観測値の出にくさを
        # 正規分布から確率をもとめその対数で算出している......?
        ...
        temp = h.clone()
        temp[i_obs] = self.gru_d(gru_input, h[i_obs]) # output h : (N, hidden_size : default=100)
        h = temp
...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1, classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, full_gru_ode=False, solver="euler", impute = True, **options):
        ...
        """
        # logvar : 分散共分散行列を非負にするためにlog処理するかどうかっぽい？

        super().__init__()

        self.impute = impute
        self.p_model = torch.nn.Sequential(
            # hidden_size=100 , p_hidden=25
            torch.nn.Linear(hidden_size, p_hidden, bias=bias),
            torch.nn.ReLU(),
            # torch.nn.BatchNorm1d(p_hidden),
            torch.nn.Dropout(p=dropout_rate),
            # Linear-->in : (N,*, p_hidden) ,out : (N,*, 2 * input_size)
            # input_size=data_train.variable_num -->96  *2=192
        ...
        # 次元を少し落とした表現を得るためにレイヤを追加
        self.classify_latent=torch.nn.Sequential(
            torch.nn.Linear(hidden_size,10,bias=bias),
            torch.nn.ReLU(),
            torch.nn.BatchNorm1d(10),
        )
        self.classification_model = torch.nn.Sequential(
            self.classify_latent,
            torch.nn.Linear(10,classification_hidden,bias=bias),
            torch.nn.ReLU(),
            torch.nn.BatchNorm1d(classification_hidden),
            torch.nn.Linear(classification_hidden,1,bias=bias)
        )
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler": # デフォルトはeuler
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
        ...
        """
        # おそらく
        # p --> [mean,logvar (or var)]　orはoptionのlogvarで操作,基本logvar
        # h -->  latent variables
        # hを単純なLinearモデルのself.p_modelに通してpを得る
        ...
        if return_path:#(default : training->return_path=False , test-> return_path=True)
            if smoother:
            ...
            if smoother: #(default: smoother=False)
...
class Discretized_GRU(torch.nn.Module):
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
        ...
        p            = self.p_model(h)  # (hidden_size, p_hidden)をうけて(p_hidden, 2 * input_size)をoutputする
        current_time = 0.0
        counter      = 0
